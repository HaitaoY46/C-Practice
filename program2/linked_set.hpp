...
used(to_copy.used)
{
    ...
    for(LN* p = to_copy.front; p != to_copy.trailer; p = p->next)
    {
        LN* nw = new LN();
        nw->value = p->value;
        f->next = nw;
...
std::string LinkedSet<T>::str() const
{
    ...
    if (used != 0) {
        for (LN* p = front; front != trailer; p = p->next)
        {
            answer << p->value << "->";
        }
    }
...
std::ostream& operator << (std::ostream& outs, const LinkedSet<T>& s)
{
    ...
    if (!s.empty()) {
        typename LinkedSet<T>::LN* p = s.front;
        for (; p->next != s.trailer; p = p->next)
        {outs <<  p->value << "->";}
        outs << p->value;
    }
...
auto LinkedSet<T>::Iterator::operator ++ () -> LinkedSet<T>::Iterator&
{
    ...
    {
        if (current ->next == ref_set->trailer)
        {
            current = ref_set->trailer;
            return *this;
        }
...
T& LinkedSet<T>::Iterator::operator *() const
{
    if (expected_mod_count != ref_set->mod_count)
        throw ConcurrentModificationError("LinkedSet::Iterator::operator *");
    if (!can_erase || current == nullptr || current == ref_set->trailer) {

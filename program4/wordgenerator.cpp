...
//int hash_queue (const WordQueue& s)
//{
//    std::hash<std::string> str_hash;
//    return str_hash(s.peek());
//}
...
////  read_corpus time                   = 4.422
////  print_corpus (sort not print) time = 0.156
//
////Array Implementations for Queue; HeapPriorityQueue, HashMap, and HashSet
////  read_corpus time                   = 4.656
////  print_corpus (sort not print) time = 0.516
//
////Briefly explain (in a comment below) what is interesting
////about the times above (and explain why it occurs).
///*
//Unexpectedly the first trial for speed ran faster than the second one, which used the hash table implementation of set.
// This is likely due to our implementation of hash set not working optimally. The reason we expected hash set and hashmap
// to work better than their array counterparts is that inserting is O(1) with hash table implementations and O(N) in the array implementation.
// We believe the call to constructors in the hash implementation was the cause of the delay because it needs to set up more than just an array.
//*/
